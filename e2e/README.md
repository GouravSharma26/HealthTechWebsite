...
1. **`e2e_fixtures.json`** doesn't exist yet — create a Django fixture (or a `manage.py` seed command, mirroring `create_sample_data.py`) with at least one verified doctor, a login you control, and an open `DoctorTimeSlot` for "today." `chat.spec.ts` and `ai-features.spec.ts` reference `seeded_doctor_username` / `seeded_doctor_password` placeholders that need to point at that fixture.
